...
summary.ClusterVAR <- function(LCVARresult,
    ...
    if(show == "Given-a-number-of-clusters" && !(Number_of_Clusters %in% LCVARresult$Call$Clusters)){
    ...
    for(K in LCVARresult$Call$Clusters)  l_LagsPerCluster[[K]] <- calculateLagList(K = K, HighestLag = max(LCVARresult$Call$Lags), LowestLag = min(LCVARresult$Call$Lags))
    ...
    if(show == "Given-a-number-of-clusters"){
        ...
        clusterCounter = which(LCVARresult$Call$Clusters == Number_of_Clusters)
        GivenClusterOutput = LCVARresult$All_Solutions_All_Clusters_Lags_Starts[[clusterCounter]]
        ...
        # The below calculation calculates the solution for (show == "Best-per-number-of-clusters") but all calculation steps are also needed if (show == "Best-overall")
            ...
            for(K in LCVARresult$Call$Clusters){
                ...
                for(LagCounter in 1:LagCombinations) {
                    for(StartCounter in 1:NumberStarts){
                        FitAllLags[LagCounter, StartCounter] = switch(TS_criterion,
                                                                      "SC" = LCVARresult$All_Solutions_All_Clusters_Lags_Starts[[ClustCount]][[LagCounter]][[StartCounter]]$SC,
                                                                      "HQ" = LCVARresult$All_Solutions_All_Clusters_Lags_Starts[[ClustCount]][[LagCounter]][[StartCounter]]$HQ)
                    }
                }
                BestRunOneK = arrayInd(which.min(FitAllLags), dim(FitAllLags))
                FunctionOutput[ClustCount, 1] = paste(LCVARresult$All_Solutions_All_Clusters_Lags_Starts[[ClustCount]][[BestRunOneK[1]]][[BestRunOneK[2]]]$Lags, collapse = " ")
                FunctionOutput[ClustCount, "log-likelihood"] = LCVARresult$All_Solutions_All_Clusters_Lags_Starts[[ClustCount]][[BestRunOneK[1]]][[BestRunOneK[2]]]$last.loglik
